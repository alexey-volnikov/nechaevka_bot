...
3. **Создайте виртуальное окружение**, чтобы зависимости не смешивались с системой:
   ...
   ```
4. **Установите все нужные модули одной командой в активированное окружение** (понадобится интернет):
   ```bash
...
- Убедитесь, что виртуальное окружение активно (в приглашении командной строки должно быть `(venv)` или `(venv)`-подобное). Если нет — активируйте его: `.venv\\Scripts\\activate` (Windows) или `source .venv/bin/activate` (WSL/Linux).
- В активном окружении выполните повторно установку всех модулей одной командой:
  ```bash
  pip install -r requirements.txt
  ```
- На Windows можно запустить готовый сценарий, который за вас создаст окружение (если его нет), активирует его и поставит пакеты:
  ```powershell
  powershell -ExecutionPolicy Bypass -File .\\install_all.ps1
  ```
  Терминал покажет шаги установки; если увидите предупреждение о политике выполнения, PowerShell запросит подтверждение.
...
Если не хотите устанавливать Python и зависимости в систему, поднимите бота в контейнере.

1. Установите [Docker Desktop для Windows](https://www.docker.com/products/docker-desktop/) и убедитесь, что демон запущен.
2. Создайте `.env` в корне (как в инструкции выше). Файл не попадет в образ, но будет передан в контейнер.
3. Запуск одним шагом через Docker Compose (сборка + старт):
   ```bash
   docker compose up --build
   ```
   - Если порт 8000 занят, поменяйте левое число в `docker-compose.yml` на свободное, например `"8001:8000"`.
4. Ручной вариант (если хотите контролировать сборку):
   ```bash
   docker build -t nechaevka_bot_image .
   docker run --env-file .env -p 8000:8000 --name nechaevka_bot nechaevka_bot_image
   ```
5. Откройте `http://127.0.0.1:8000` и смотрите `/` и `/stats`. Остановить контейнер можно `docker compose down` или `docker stop nechaevka_bot`.

### Зачем Docker в этом проекте
- Изоляция: весь код и зависимости в одном образе, ничего не ставится в систему.
- Повторяемость: на любой машине с Docker получите одинаковое окружение.
- Простая очистка: удалите контейнер/образ — и следов нет.
