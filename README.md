...
- Показывает последние сообщения (peer_id, from_id, текст) в таблице дашборда.
...
- Ведет системный журнал (SQLite) с пояснениями на русском и отдельной страницей `/system-logs` для просмотра и очистки.
...
5. **Создайте собственный `.env`, не коммитя его в git**:
   ...
   - Заполните файл без кавычек:
     ...
     PORT=8000
     DEMO_MODE=0
     SYSTEM_LOG_DB=system_events.db
     ```
   ...
   - `SYSTEM_LOG_DB` можно не менять: по умолчанию файл `system_events.db` создается рядом с `app.py` и хранит системные события.
...
3. Откройте `http://127.0.0.1:8000`:
   ...
   - Карточка «Системные уведомления» показывает количество записей и индикатор новых предупреждений/ошибок.
   - `/system-logs` — отдельная страница со всеми записями журнала и кнопкой очистки.
   ...
   - `/api/system-logs` — получение журнала (параметр `mark_read=false` оставляет новые события непрочитанными).
   - `/api/system-logs/clear` (POST) — очистка журнала системных событий.
   - `/api/system-logs/summary` — краткая сводка счетчиков для индикатора на дашборде.
...
- Мешают логи HTTP-запросов: в `app.py` уровень `werkzeug` поднят до WARNING, поэтому мелкие запросы не захламляют вывод.
...
- Расширить сервисный журнал фильтрами, поиском и экспортом CSV прямо из `/system-logs`.
