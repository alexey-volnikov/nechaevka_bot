...
## Что умеет
- Читает события из Long Poll и ведет живую статистику (сообщения, активные участники).
...
- Визуализирует динамику активности (сообщения + участники) на графике (Chart.js) и обновляет каждые 3 секунды.
...
- Если файл не сохранился (нет прямой mp4-ссылки или скачивание упало), показывает красный бейдж и оставляет ссылку на плеер VK, чтобы сразу понять проблему и открыть вложение вручную.
...
5. В главном экране останутся карточки «Активные участники» и «Сообщения», так что интерфейс станет просторнее и удобнее.
...
- Значение по умолчанию для графика задаётся переменной окружения `TIMELINE_DEFAULT_MINUTES` (если не указана, берётся 60 минут), переключатель есть прямо на главной странице.
