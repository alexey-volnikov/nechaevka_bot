...
- Позволяет выбрать конкретный `peer_id` и смотреть логи только по нужному чату.
...
- Таблица `events` можно открыть через любой SQLite-клиент или встроенную панель VS Code.
