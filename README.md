...
5. Скопируйте файл `.env.example` в `.env` и впишите данные своего сообщества:
   ...
   ```
   - **VK_GROUP_TOKEN** — токен доступа для вашего сообщества ВК с правами «Сообщения сообщества». Его можно получить в настройках сообщества → Разработка → Создать ключ.
   - **VK_GROUP_ID** — числовой идентификатор вашего сообщества без префиксов `club` или `public`. Например, если адрес страницы `https://vk.com/club123456`, то `VK_GROUP_ID` будет `123456`.
   - Значения в `.env` прописываются без кавычек. Пример готового файла:
     ```
